...
- 新增展示层 Python tokenizer，渲染关键字、字符串、注释、数字、布尔值、内置名称、函数调用、运算符和标点。
- 代码区改为可滚动的只读高亮视图，保留逐字复制和原始换行。
- 不新增高亮依赖，不使用 HTML 字符串注入；无法识别的内容回退为普通文本。
- 自动化测试已覆盖 token 文本保持、多行字符串、中文注释和未闭合字符串回退；尚待桌面端人工视觉检查。
