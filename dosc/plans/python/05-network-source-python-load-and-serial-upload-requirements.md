...
远程拓展已经同时配置 Gitee 和 GitHub 来源，但顺序固定为 Gitee 优先。只有当前来源失败后，才会尝试下一个来源。

入口位于 `packages/scratch-gui/src/lib/custom-extension/remote-library-client.js`：

- `DEFAULT_REMOTE_CATALOG_SOURCES` 定义默认来源顺序。
- `loadRemoteLibraryCatalog` 按顺序读取远程目录。
...
4. 根据固件命令长度和稳定性测试 256、512、1024 字节分块，避免固定小块造成大量串口往返。
