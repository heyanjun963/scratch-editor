...
| Python 代码编辑器高亮 | 已实现初版 | 代码区已增加展示层 Python 词法高亮，保持文本内容、加载、保存和运行行为不变；待人工视觉检查 | P1 |
...
输出和上传记录：[2026-07-17 串口硬件输出、控制台拖拽与 Raw REPL 上传执行记录](./2026-07-17-serial-output-and-console-resize-progress.md)。

目标：桌面端获取本机串口、连接指定设备，通过 Raw REPL 写入并校验生成的 `main.py`，同时显示设备输出。
...
1. Electron 主进程负责 Web Serial 权限、候选设备选择和页面安全边界。
2. Renderer 使用 `navigator.serial` 打开和写入串口；preload 只转发候选端口列表。
3. 串口参数需要明确：baudRate、dataBits、stopBits、parity。
4. 上传过程检查设备响应、超时和失败；取消与可视化进度后续补齐。
...
- AI 机甲产品当前使用 MicroPython Raw REPL 写入 `main.py`；其他产品仍需按固件选择文件复制、厂商工具或 bootloader 协议。

本轮实现范围：

1. Electron 主进程处理 `select-serial-port`，把完整候选项发给渲染进程展示，并暂时自动选择第一项。
...
6. 连接后持续读取 `port.readable`，按文本行批量显示硬件输出。
7. 断开和标签页卸载时先释放 reader lock，再关闭串口。
8. Python 控制台支持鼠标拖拽和键盘调节高度。
9. Upload 通过 Raw REPL 分块写 `main.py`，检查命令响应和设备端文件大小后才显示成功。
10. 上传成功后软复位设备，新 `main.py` 的启动输出继续显示在控制台。
...
- 暂无上传取消、重试和可视化进度 UI。
- 暂无设备型号、固件版本识别和多产品协议适配。
- 当前校验设备端文件大小；文件内容哈希和上传后业务行为仍需真机确认。
- 可读流结束后会恢复断开状态，但尚未监听 `navigator.serial` 全局 disconnect 事件，需要真机验证不同驱动的拔线行为。
- 当前串口权限和设备授权边界需要在产品化时进一步收紧。
- 较大代码、中文代码、传输中拔线和设备存储不足仍需真机测试。
...
连接成功后状态变为已连接
硬件输出实时进入 Console
拖动分隔条可以调整 Console 高度
点击上传代码
显示 Uploading；只有设备端 main.py 字节数校验通过才显示成功
设备软复位后控制台显示新 main.py 输出
断开串口后状态正确恢复
```
...
插入 USB 串口设备
点击 Connect
下拉框显示 Electron 返回的完整串口候选项
选择波特率 115200
在下拉框选择非第一项，点击 Connect 后实际连接所选端口
终端输出 connected
硬件主动输出日志时终端实时显示
...
连接串口
点击 Upload
终端先输出 Uploading and verifying
校验成功后输出 Wrote and verified main.py
用设备文件工具确认 main.py 内容已经替换
...
用例 6：控制台调节
上下拖动 Console 上方分隔条
代码区至少保留 160px，控制台至少保留 120px
拖动过程中终端自动适配宽高，已有输出不消失

用例 7：硬件拔出
连接后让硬件持续输出日志
直接拔出 USB
控制台不白屏，连接状态恢复为断开，端口资源被释放
```

### 阶段 H：自定义扩展库和配置导出

当前状态：已提交 MVP，远程更新和项目版本锁定未开始。
...
- 支持 `.json/.zip/.sbext` 本地导入、动态注册和 Python 模板产码。
- 支持目录包携带 `blocks.json`、`generator/python.json` 和 Python 运行库。
- AI机甲麦轮车已从内置 JS manifest 外置为标准源包。
- 已有通用打包脚本生成可重新导入的 `aimecanum.sbext`。
- 已有打包结构和重新导入等价性单元测试。
- 本地导入包已迁入独立“用户拓展”分类，支持加载、卸载和删除。
- 用户包启用状态可持久化，旧 manifest-only 数据自动按已加载迁移。
- 产品包来源已区分内置默认、后台目录、远程缓存和用户导入；离线优先使用最近一次已校验缓存，内置旧版本只做最后兜底。

仍缺：

- 产品拓展页直接导出 `.sbext`。
- GitHub/Gitee Release 查询、下载、SHA256 校验和缓存。
- 项目保存拓展版本、来源和 checksum，加载旧项目时恢复固定版本。
