...
| 串口和上传代码 | Web Serial MVP 已接入 | 静态审查问题已记录，待取得产品后修复端口选择和状态恢复，并实现真实设备上传协议 | P1 |
...
当前状态：Web Serial MVP 已接入；2026-07-15 已完成静态审查，问题记录待取得产品和上传协议后处理。
...
- 串口下拉框当前只更新显示状态，Connect 重新触发 `requestPort()` 后主进程仍自动选择第一项。
- `writer.write()` 成功只表示文本写入串口流，不表示设备保存或执行成功。
- close/write 失败或 USB 拔出后，连接状态缺少统一复位和 `disconnect` 事件处理。
- 当前串口权限和设备授权边界需要在产品化时进一步收紧。
- 上述问题暂不修改，等待目标产品、固件和上传协议到位后统一处理。
...
选择波特率 115200
终端输出 selected；当前会自动连接候选列表第一项，后续产品测试时需要验证并修复指定端口选择
终端输出 connected
